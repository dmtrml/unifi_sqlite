...
## Недавние обновления
- Удобный выбор даты (`react-datepicker`) с ручным вводом и dropdown месяца/года; стили адаптированы под светлую/тёмную тему.
- Отображение дат нормализовано через UTC-ключи, чтобы не было смещения на сутки в разных таймзонах.
- При дублировании переводов доступны поля `Amount Sent/Received`, изменённые суммы сохраняются корректно.
- Автоподбор иконок категорий по словарю, обновлённые списки и скрипт массового применения иконок.
