...
- В разделе Settings → Data Management появились кнопки экспорта CSV/JSON и полного JSON-бэкапа (скачивание/восстановление через `/api/backup`).
...
- Новый удобный выбор даты на `react-datepicker` с ручным вводом и выпадающими месяцами/годами; визуал под нашу светлую/тёмную тему.
- Даты в списках/отчётах форматируются через UTC-ключи, чтобы не сдвигались на сутки в разных таймзонах.
- В дублировании переводов появились поля `Amount Sent/Received`, изменённые суммы корректно сохраняются.
- Автоподбор иконок категорий по словарю (при создании/импорте), выпадающие списки обновлены; есть скрипт массового применения.
...
- Новый API `/api/backup` — полный JSON-бэкап пользователя (accounts/categories/budgets/transactions/recurring) и восстановление с учётом последовательности и фиксации балансов; настройки вызывают этот API и предупреждают о полной перезаписи данных.
